...
const { Scheduler, DeleteScheduleCommand} = require("@aws-sdk/client-scheduler");
const {EventBridgeClient, DeleteRuleCommand,DeleteApiDestinationCommand, RemoveTargetsCommand} = require("@aws-sdk/client-eventbridge");
...
module.exports= class extends Generator {
  async prompting(){
    ...
    const schedulesList=(await (schedulerClient.listSchedules(input))).Schedules.map((schedule)=>schedule.Name).filter((value)=>{
  ...
  async writing(){
    ...
    const deleteScheduleCommand=new DeleteScheduleCommand(deleteScheduleCommandInput);
    ...
    const apiDestinationDeleteCommand=new DeleteApiDestinationCommand(ApiDestinationDeleteCommandInput);
    const ruleDeleteCommand=new DeleteRuleCommand(removeRuleCommandInput);
    const targetRemoveCommandInput={
      Rule:ruleName,
      Ids:[targetName],
    }
    const targetRemoveCommand=new RemoveTargetsCommand(targetRemoveCommandInput);

    try{
      const responseTarget=await eventBridgeClient.send(targetRemoveCommand);
      const responseApiDestination= await eventBridgeClient.send(apiDestinationDeleteCommand);
      const responseRule=await eventBridgeClient.send(ruleDeleteCommand);
      const responseScheduler=await schedulerClient.send(deleteScheduleCommand);
    } catch (error){
      console.log(error)
    }
